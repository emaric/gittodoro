...
describe("[ViewDurationDetailsCommand] unit tests", () => {
  describe("when trying to execute view duration details command", () => {
    it("should get a duration response", () => {
      const sampleDuration = {id: 0, pomodoro: 50, short: 10, long: 15, longInterval: 4}
      ...
      const viewDurationDetailsCommand = new ViewDurationDetailsCommand(testDataGateway, testPresenter);
      ...
      const expectedOutput = 'This is the response: ' + JSON.stringify(sampleDuration)
